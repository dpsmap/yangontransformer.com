	...
	public function getParamTabsList( $atts, $content ) {
		...
		if ( ! $isPageEditabe ) {
			...
			foreach ( WPBakeryShortCode_VC_Tta_Section::$section_info as $nth => $section ) {
				...
				$tab_img = wp_get_attachment_image_src($section['tabimg'], 'thumbnail');
				...
				{
					$tab_icon = urldecode($tab_img[0]);
					$a_html = '<a href="#' . $section['tab_id'] . '" data-vc-tabs data-vc-container=".vc_tta"><span class="tab_img_icon"><img src="'.$tab_icon.'"></span>' .  $title . '</a>';
					$html[] = '<li class="' . implode( ' ', $classes ) . '" data-vc-tab>' . $a_html . '</li>';
				}
